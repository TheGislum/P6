...
def main(model_version, model_colour, model_Path, use_fast_detection, calibrate, camera_resolution, verbose):
    ...
    if use_fast_detection:
        ...
        pose = PoseEstimation() #TODO update to fast and iniitiate with frame
    else:
        face = FaceTracking()
        pose = PoseEstimation()
    ...
    if calibrate:
        ...
        calibration = Calibration(model=model_Path, model_colour=model_colour, version=model_version,
                                    fast=use_fast_detection, verbose=verbose) #TODO fix used model and fast selection
    ...
    while True:
        ...
        if ret == True:
            if face.refresh(frame):
                ...
                if verbose:
                    print(f"model estimates:         X={x-offset_x} Y={y-offset_y}")
                    print(f"model estimate + offset: X={x} Y={y}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_colour", type=bool, default=False, help="use colour images (default: False)")
    parser.add_argument("--model_Path", type=str, default="./garage/03.pth", help="gaze model path (default: 03)")
    parser.add_argument("--use_fast_detection", type=bool, default=False, help="use fast face/landmark detection (default: False)")
    parser.add_argument("--calibrate", type=bool, default=True, help="run calibration (default: True)")
    parser.add_argument("-cr", "--camera_resolution", type=tuple, default=(10000, 10000), help="camera resolution input to model (default: max)")
    parser.add_argument("-v", "--verbose", action="store_true")

    args = parser.parse_args()

    main(args.model_version, args.model_colour, args.model_Path, args.use_fast_detection, args.calibrate, args.camera_resolution, args.verbose)
